...
const responseSchema = {
    ...
    items: {
        type: "OBJECT",
        properties: {
            "nome": { "type": "STRING", "description": "Nome da matéria escolar (ex: álgebra, verbo intransitivo)." },
            "descricao": { "type": "STRING", "description": "Descrição concisa da matéria escolar." },
            "data_criacao": { "type": "STRING", "description": "Ano de aprendizado da matéria escolar (ex: '2° ano do fundamental')." },
            "link": { "type": "STRING", "description": "URL oficial do toda matéria ou brasil escola." },
            "tags": {
                "type": "ARRAY",
                "description": "Array de 3 a 5 strings que categorizam a matéria escolar (ex: 'números', 'verbos', 'aceleração', 'meio ambiente').",
                "items": { "type": "STRING" }
            }
        },
...
async function generateNewKnowledge(existingKnowledge) {
    ...
    const systemPrompt = `Você é um especialista em conhecimentos escolares, assim como um professor. Sua tarefa é criar ${TOTAL_ITEMS} novas entradas sobre diferentes matérias escolares (matemática, física, português, geografia, biologia, etc.) com a mesma estrutura JSON. Garanta que cada entrada seja única e relevante. O foco é em termos atuais e amplamente importantes em vestibulares.`;
    
    // NOVO userQuery: Focado em 25 itens e evitando nomes existentes
    const userQuery = `Gere uma lista de ${TOTAL_ITEMS} novas matérias. Siga estritamente a estrutura JSON e o requisito de ser um ARRAY com EXATAMENTE ${TOTAL_ITEMS} objetos. NÃO use NENHUM dos seguintes nomes: ${existingNames}.`;
